...
export namespace FightSystem{
    ...
    function doFight(damager:Damager,hper:Hper){
        ...
        (damager.emitTarget || damager.node).emit(Event.BeatHper, beatData);
        (hper.emitTarget || hper.node).emit(Event.Beaten, beatData);
        if(hper.hp <= 0){
            (damager.emitTarget || damager.node).emit(Event.KillHper, beatData);
            (hper.emitTarget || hper.node).emit(Event.Killed,beatData);
        }
